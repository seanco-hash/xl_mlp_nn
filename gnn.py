...
class HGPSLGNN_NotConnected(torch.nn.Module):
    def __init__(self, n_feat, n_hidden=128, out_size=1, pool_ratio=0.8, dropout=0.0, sample=False,
        ...
        self.HGPSLGNN_a = HGPSLGNN(n_feat, n_hidden // 2, out_size, pool_ratio, dropout, sample, sparse,
                                   structure_learn, lamb, include_linear=False)
    ...
    def forward(self, data):
        cb_out, omega_out, theta_out, phi_out = None, None, None, None
...
class HGPSLGNN(torch.nn.Module):
    def __init__(self, n_feat, n_hidden=128, out_size=1, pool_ratio=0.8, dropout=0.0, sample=False,
                 sparse=True, structure_learn=True, lamb=1.0, include_linear=True):
...
def create_model(cfg):
    # return SimpleGCN(cfg['DATA']['NODE_FEAT'], cfg['MODEL']['OUTPUT_SIZE'])
    if cfg['DATA']['IS_CONNECTED']:
        return HGPSLGNN(cfg['DATA']['NODE_FEAT'], out_size=cfg['MODEL']['OUTPUT_SIZE'],
                        n_hidden=cfg['MODEL']['LAYERS'], dropout=cfg['MODEL']['DROPOUT'],
                        sample=cfg['MODEL']['SAMPLE'], pool_ratio=cfg['MODEL']['POOL_RATIO'])
    return HGPSLGNN_NotConnected(cfg['DATA']['NODE_FEAT'], out_size=cfg['MODEL']['OUTPUT_SIZE'],
                                 n_hidden=cfg['MODEL']['LAYERS'], dropout=cfg['MODEL']['DROPOUT'],
                                 sample=cfg['MODEL']['SAMPLE'], pool_ratio=cfg['MODEL']['POOL_RATIO'],
                                 cb_head=cfg['MODEL']['CB_PRED'], omega_head=cfg['MODEL']['OMEGA_PRED'],
                                 theta_head=cfg['MODEL']['THETA_PRED'], phi_head=cfg['MODEL']['PHI_PRED'])
